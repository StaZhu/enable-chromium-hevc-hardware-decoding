...
2. (Optional) To enable HEVC software decoding: switch to `src/third_party/ffmpeg` dir, then execute `git am /path/to/add-hevc-ffmpeg-decoder-parser.patch`. If failed to apply the patch, could also try `node /path/to/add-hevc-ffmpeg-decoder-parser.js` to enable software decoding (Node.js is required to run the script), then switch to `src` dir, and execute `git am /path/to/enable-hevc-ffmpeg-decoding.patch`, then execute `git am /path/to/change-libavcodec-header.patch` manually (this may still fail if the upstream got updated but this repo doesn't sync the code in time, you could submit an issue or patch to report or fix this).
