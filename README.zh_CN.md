...
*注3：从 Polaris 架构开始，AMD GPU 仅支持硬件解码 HEVC 8 ~ 10b 420 的内容。*

*注4：尽管 NVIDIA GPU 支持 8 ~ 12b 非 422 HEVC Rext CUVID 或 NVDEC 硬解码，但由于 NVIDIA 没有给 D3D11 接口暴露这部分能力，因此 Chromium 以后也不会支持它们。*

*注5：Windows HEVC 8b 444, 12b 422, 12b 444 支持需要 Chrome >= `117.0.5866.0`。*

*注6：Windows HEVC 8b 422 支持需要 Chrome >= `118.0.5956.0`。*

*注7：保留原始 4:2:2/4:4:4 色度抽样, 需要 Chrome >= `125.0.6408.0`。*
