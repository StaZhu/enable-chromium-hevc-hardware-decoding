...
视频编码：支持 WebCodec (支持 macOS, Windows, Android, Chrome >= `130.0.6703.0` 版本默认启用), WebRTC (测试方式见视频解码部分描述)，MediaRecorder (Windows, macOS, and Android, Chrome 136 默认启用，Chrome 133 ~ 135 版本，需传入 `--enable-features=MediaRecorderHEVCSupport` 启用，[Demo](https://webrtc.github.io/samples/src/content/getusermedia/record/))。
