...
2. (可选) 支持 HEVC 软解：切换到 `src/third_party/ffmpeg` 目录，执行 `git am /path/to/add-hevc-ffmpeg-decoder-parser.patch` 。如果有冲突，也可尝试使用 `node /path/to/add-hevc-ffmpeg-decoder-parser.cjs` 直接修改代码（需要确保Node.js已安装再执行该命令）, 然后继续执行 `git am /path/to/change-libavcodec-header.patch` (如果本仓库同步上游不及时，这条命令也可能失败，如遇失败可提 Issue 反馈，或直接提交修复的 Merge Request)，最后切换回 `src` 目录，执行 `git am /path/to/enable-hevc-ffmpeg-decoding.patch`。
